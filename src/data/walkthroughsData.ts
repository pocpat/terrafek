  ...
  lifecycle {
    ...
    ignore_changes  = [tags["LastScanned"]]
      ...
      },
      {
        ...
        explanation:
          "Every resource type has provider-specific REQUIRED attributes — omit them and `terraform plan` fails before any cloud API call. This step covers four habits every AWS resource block should have.\n\n**1. Tags — label everything.**\n- The `tags = { ... }` map labels every resource for the **AWS console**, **cost reports**, and **filtering**.\n- Adopt a convention:\n  - `Name` — identifies the resource (e.g. \"web-server-prod\")\n  - `Environment` — marks the stage (\"production\", \"staging\", \"dev\")\n  - `ManagedBy = \"Terraform\"` — flags IaC-managed resources so manually-built ones stand out\n\n**2. Security group ingress — open ports deliberately.**\n- Each `ingress` block opens **one port range** and needs **four arguments**: `from_port`, `to_port`, `protocol`, `cidr_blocks`.\n- Port **80** = HTTP, port **443** = HTTPS.\n- `cidr_blocks = [\"0.0.0.0/0\"]` means **\"allow from any IP\"** — fine for a public web server, **dangerous for a database**.\n\n**3. List arguments use `[]` brackets.**\n- Attributes ending in `_ids` (plural) take a **list**, not a single value.\n- An instance can attach to several security groups: `[aws_security_group.web_sg.id]` for one, `[aws_security_group.web_sg.id, aws_security_group.db_sg.id]` for two. **The brackets are what make it a list.**\n\n**4. `map_public_ip_on_launch` — who gets a public IP.**\n- Set on `aws_subnet`; controls whether EC2 instances launched there receive a **public IP**.\n- `true` → **public subnets** (internet-facing) · `false` → **private subnets** (databases, internal services).\n\n**Bonus habit:** always add a `description` to **variable blocks** — it documents intent for teammates and surfaces in tooling.",
        objectives: [
